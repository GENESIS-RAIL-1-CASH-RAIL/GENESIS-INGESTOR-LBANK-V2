// src/bootstrap.ts — SF-Grade v2.0.0-sf Ingestor: LBANK_V2 (DEX)
...
function extractPrices(json: any): Record<string, { bid: number; ask: number; ts: number }> {
  ...
  if(Array.isArray(items)){for(const t of items){const sym=t.symbol||t.pair||t.name||t.market||t.instrument_name||"";let bid:number,ask:number;const latest=parseFloat(t.ticker?.latest||t.price||t.lastPrice||"0");if(t.bidPrice||t.bid||t.best_bid){bid=parseFloat(t.bidPrice||t.bid||t.best_bid||"0");ask=parseFloat(t.askPrice||t.ask||t.best_ask||t.price||"0");}else{bid=latest;ask=latest;}if(sym&&!isNaN(bid)&&!isNaN(ask)&&isFinite(bid)&&isFinite(ask)&&bid>0&&ask>0){const n=normalizePair(sym);prices[n]={bid,ask,ts:now};priceBook[n]={bid,ask,ts:now};}}}
